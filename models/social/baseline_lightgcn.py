...
class BASELINE_LIGHTGCN(BaseModel):
    ...
    def _coo_to_sparse_tensor(self, coo_mat):
        ...
        shape = coo_mat.shape
        return t.sparse_coo_tensor(indices, values, shape).coalesce()
    ...
    def _create_adj_matrix(self, trn_mat):
        ...
        A = t.sparse_coo_tensor(combined_indices, combined_values, (total_num, total_num)).coalesce()
        return A
    ...
    def _normalize_sparse_matrix(self, mat):
        ...
        normalized_mat = t.sparse_coo_tensor(indices, norm_values, mat.size())
    ...
    def _propagate(self, combined_adj, embeds):
        ...
        combined_adj = combined_adj.to(self.device)
    ...
    def forward(self, keep_rate):
        ...
        if self.is_training:
            adj = self.edge_dropper(self.adj, keep_rate)
            trust_adj = self.edge_dropper(self.trust_adj, keep_rate)
