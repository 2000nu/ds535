...
class DataHandlerSocial:
	...
	def _sparse_mx_to_torch_sparse_tensor(self, sparse_mx):
		...
		return t.sparse_coo_tensor(indices, values, shape, dtype=t.float32, device=configs['device'])
	...
	def _build_motif_induced_adjacency_matrix(self, trust_mat, trn_mat):
		...
		row_sums = H_s.sum(axis=1).reshape(-1, 1)
		row_sums[row_sums == 0] = 1  # Prevent division by zero
		H_s = H_s.multiply(1.0 / row_sums)
		
		H_j = sum([A8, A9])
		# H_j = H_j.multiply(1.0 / H_j.sum(axis=1).reshape(-1, 1))
		row_sums_j = H_j.sum(axis=1).reshape(-1, 1)
		row_sums_j[row_sums_j == 0] = 1  # Prevent division by zero
		H_j = H_j.multiply(1.0 / row_sums_j)
		
		H_p = A10
		H_p = H_p.multiply(H_p > 1)
		# H_p = H_p.multiply(1.0/H_p.sum(axis=1).reshape(-1, 1))
		row_sums_p = H_p.sum(axis=1).reshape(-1, 1)
		row_sums_p[row_sums_p == 0] = 1  # Prevent division by zero
		H_p = H_p.multiply(1.0 / row_sums_p)
	...
	def _build_joint_adjacency(self, trn_mat):
		...
		norm_adj = t.sparse_coo_tensor(indices, values, shape)
		return norm_adj
